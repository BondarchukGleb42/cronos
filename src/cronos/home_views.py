...
def main_panel(overview: dict | None = None) -> dict:
    ...
    text += (
        "\n\nИнициатива включена: могу уместно вернуться к обсуждённому делу. Выключить можно в настройках или сообщением."
        if overview.get("proactivity", True)
        else "\n\nИнициатива выключена: сам писать первым не буду. Явные напоминания продолжают работать."
    )
    text += " Всё доступно обычными сообщениями; меню помогает начать."
...
def guide_panel() -> dict:
    return _panel(
        ...
        "инициатива включена по умолчанию: могу уместно вернуться к обсуждённому делу. "
        "Её можно выключить в настройках или сообщением.\n\n"
...
def settings_panel(preferences: dict) -> dict:
    proactive = bool(preferences.get("proactivity", True))
    return _panel(
        "⚙️ Настройки\n\n"
        f"Часовой пояс: {_line(preferences.get('timezone'), 100, 'не указан')}\n"
        f"Стиль общения: {_line(preferences.get('tone'), 200, 'не указан')}\n"
        f"Писать первым: {'включено' if proactive else 'выключено'}\n\n"
        "Инициатива включена по умолчанию; твоё явное отключение сохраняется. "
        ...
        [
            [
                _button(
                    "Выключить инициативу" if proactive else "Включить инициативу",
