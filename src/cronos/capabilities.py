...
SKILLS = {
    ...
    "topics": {
        ...
        "instructions": "Новый разговор создаётся кнопкой «Новый чат», командой /new или через topic_create по просьбе пользователя. После первого вопроса и ответа маленькая модель автоматически даёт теме название; затем обновляет его на каждые 10 сообщений пользователя и ассистента. Ручное переименование в Telegram закрепляет название. Список и переключение чатов — штатный список тем Telegram; /chats и topics_list показывают сохранённые темы. Не обещай автоматически переключить интерфейс Telegram. При недоступных private topics честно сообщи об этом; обычный чат продолжит работать.",
    },
    "account": {
