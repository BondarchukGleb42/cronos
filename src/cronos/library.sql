-- Explicit function schemas also work under PostgreSQL index-maintenance search_path.
...
CREATE OR REPLACE FUNCTION public.cronos_library_fold(value text) RETURNS text
...
CREATE OR REPLACE FUNCTION public.cronos_library_message_body(value jsonb) RETURNS text
LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
...
CREATE OR REPLACE FUNCTION public.cronos_library_project_body(name text, goal text, state jsonb)
...
CREATE OR REPLACE FUNCTION public.cronos_library_vector(config regconfig, title text, body text)
RETURNS tsvector LANGUAGE sql IMMUTABLE PARALLEL SAFE AS $$
  SELECT setweight(to_tsvector(config,public.cronos_library_fold(title)),'A') ||
         setweight(to_tsvector(config,public.cronos_library_fold(body)),'D')
$$;

CREATE INDEX IF NOT EXISTS messages_library_simple ON messages USING gin
  (public.cronos_library_vector('simple'::regconfig,'',public.cronos_library_message_body(content))) WHERE NOT excluded;
CREATE INDEX IF NOT EXISTS messages_library_russian ON messages USING gin
  (public.cronos_library_vector('russian'::regconfig,'',public.cronos_library_message_body(content))) WHERE NOT excluded;
CREATE INDEX IF NOT EXISTS artifacts_library_simple ON artifacts USING gin
  (public.cronos_library_vector('simple'::regconfig,filename,coalesce(extracted->>'text','')));
CREATE INDEX IF NOT EXISTS artifacts_library_russian ON artifacts USING gin
  (public.cronos_library_vector('russian'::regconfig,filename,coalesce(extracted->>'text','')));
CREATE INDEX IF NOT EXISTS projects_library_simple ON projects USING gin
  (public.cronos_library_vector('simple'::regconfig,name,public.cronos_library_project_body(name,goal,state))) WHERE NOT context_excluded;
CREATE INDEX IF NOT EXISTS projects_library_russian ON projects USING gin
  (public.cronos_library_vector('russian'::regconfig,name,public.cronos_library_project_body(name,goal,state))) WHERE NOT context_excluded;
