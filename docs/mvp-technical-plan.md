...
Актуально на 08.09.2026: инициатива включена по умолчанию у новых и существующих пользователей без ранее выраженного отключения. Cronos не просит повторного общего разрешения и сохраняет явный выбор выключить инициативу. Полезное продолжение опирается на текущий пользовательский контекст и оформляется отдельным заданием; наличие проекта или включённого режима само по себе не создаёт расписание. Вне проекта остаются короткие check-in, для активного проекта — подготовленная полезная инициатива с ограниченным набором инструментов. Покупки, рассылки другим людям и внешние действия этим режимом не разрешаются. Явная просьба напомнить разрешает конкретное действие независимо от общего режима инициативы.
...
Актуальный разговорный маршрут с **08.09.2026** — `openai/gpt-5.6-luna` по умолчанию для всех тарифов; пользователь может выбрать `openai/gpt-5.6-terra` или `openai/gpt-5.6-sol`. Настройки общие для аккаунта и применяются к новым запросам во всех темах. Та же выбранная модель обрабатывает текст, изображения и инструмент анализа сохранённых изображений/страниц PDF; повтор запроса не меняет модель. Поиск (`MODEL_SEARCH`, Sonar), генерация изображений (`MODEL_IMAGE`, Gemini) и названия тем (`MODEL_TITLE`, Llama) — отдельные маршруты. Текущий список и нормализация предпочтений находятся в `src/cronos/model_preferences.py`, параметры провайдера — в `src/cronos/settings.py`.

**Историческая таблица кандидатов исследования от 05.09.2026; не текущая конфигурация.** После той проверки временно использовались DeepSeek V4 Flash для диалога и Qwen для vision. Таблица сохраняется как основание первоначального выбора.
...
Текущие режимы — «Без рассуждения» (по умолчанию) и «Глубокое рассуждение»: для выбранной GPT-модели они передаются как `reasoning_effort=none/high`. Сложность задачи не включает глубокий режим автоматически. При выключенном режиме `deep_reason` исключён из схемы инструментов и проверяет запрет при исполнении. Выбор модели и режима открывается из нижней сворачиваемой панели; он не зависит от тарифа и не предлагается при каждом обычном ответе.

Встроенные повторы SDK отключены. Не более трёх обращений приложения в одной цепочке восстановления: основная попытка и два ограниченных повтора. Для Luna/Terra/Sol все попытки сохраняют выбранную модель и режим; скрытой замены другим диалоговым маршрутом нет. У каждого run общий бюджет шагов/времени; точные рабочие значения определяются smoke-проверкой. Сетевой timeout после отправки не является основанием немедленно сделать дорогой дубль. Число внутренних upstream-попыток агрегатора отдельно не гарантируется приложением.
