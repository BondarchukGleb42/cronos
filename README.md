...
Для Cronos включён Topics Mode (`has_topics_enabled=true`). Кнопка и `/new` создают темы от имени бота. Самостоятельное создание штатной кнопкой Telegram — отдельная настройка BotFather; на снимке 6 сентября 2026 она выключена (`allows_users_to_create_topics=false`). Подробности и результаты проверки — в [docs/topics.md](docs/topics.md).

Версия `8722f04b18901d03ff40ba05fcdd9d0e8aa2e3b7` развёрнута: [CI](https://github.com/BondarchukGleb42/cronos/actions/runs/34053942382) завершился успешно, включая 219 pytest-тестов и 9 проверок kubeconfig. В рабочей среде подтверждены создание темы, доставка приветствия и ответа с кнопками, а также фоновое название после первой пары сообщений. Все пять постоянных pod готовы, перезапусков нет, gateway `/readyz` возвращает `ready`.
