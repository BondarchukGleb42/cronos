...
Нажми **Start**: появится отдельный чат **🪐 Cronos** с закреплённым приветствием и главным меню. «➕ Новый чат» запускает новый разговор; остальные кнопки показывают чаты, задачи, тариф, гайд, общую память и настройки. `/menu` или сообщение «меню» возвращает к этому экрану. Внутри домашней темы тоже можно общаться обычным текстом.
...
`/menu`, `/new`, `/chats`, `/tasks`, `/plans`, `/help`, `/memory`, `/settings`, `/stop`, `/delete`, `/clearall` — дополнительные короткие команды. Большая часть управления доступна свободным текстом. Голосовые сообщения и реальная оплата будут позже. Устройство главного меню — в [docs/home.md](docs/home.md).

## Несколько чатов

Отдельные разговоры реализованы через native private topics Telegram. `/new` и кнопка «➕ Новый чат» создают тему; `/chats` показывает сохранённые названия, а переключение происходит в штатном списке тем Telegram. История у каждой темы своя, память и предпочтения пользователя общие.
...
Кнопка «🗑 Удалить этот чат», `/delete` или просьба «удали этот чат» сначала показывают подтверждение, затем удаляют тему, её историю и задания. Общая память и библиотека файлов сохраняются. `/clearall` или «полностью очисти мои данные» удаляют также память, настройки, файлы и все остальные чаты. Тариф, баланс и учтённые расходы сохраняются. Подтверждение действует 15 минут; отмена доступна кнопкой или словом «отмена».
