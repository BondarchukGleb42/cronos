...
- **Инициатива:** включена по умолчанию; явное отключение сохраняется. Cronos выбирает полезные продолжения по текущему контексту: короткий check-in вне проекта или проверка с подготовленным результатом в активном проекте. Действуют разрешённые инструменты, тихие часы, частота, бюджет и отмена. Включённый режим сам не создаёт заданий и не разрешает покупки или рассылки другим людям.
...
- Coordinator проверяет отмену/перенос перед отправкой. Для инициативных сообщений действуют актуальный переключатель, тихие часы и суточная частота. Выключение инициативы блокирует самостоятельные обращения; явные напоминания исполняются в назначенное время.
