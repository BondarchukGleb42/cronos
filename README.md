...
Нажми **Start**: появится отдельный чат **🪐 Cronos** с закреплённым приветствием и главным меню. «➕ Новый чат» запускает новый разговор; экран показывает активные проекты, следующие шаги и недавние результаты. Остальные кнопки показывают чаты, задачи, тариф, гайд, память и настройки. `/menu` или сообщение «меню» возвращает к этому экрану. Внутри домашней темы тоже можно общаться обычным текстом.
...
Отдельные разговоры реализованы через native private topics Telegram. `/new` и кнопка «➕ Новый чат» создают тему; `/chats` показывает сохранённые названия, а переключение происходит в штатном списке тем Telegram. История у каждой темы своя. Память может относиться ко всему аккаунту, проекту или одному чату; временным фактам можно задать срок. Проект связывает несколько чатов только по явному запросу.
